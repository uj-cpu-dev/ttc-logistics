...
import {useNavigate, useSearchParams} from "react-router-dom"; // 👈 Import this
...
export default function TrackPackage() {
  ...
  // 1️⃣ Check URL on mount for ?trackingId=...
  useEffect(() => {
    const id = searchParams.get("trackingId");
    if (id) {
      setTrackingNumber(id);
      if (id.trim() === dummyTrackingData.trackingNumber) {
  ...
  // 2️⃣ Scroll to tracking details when available
  useEffect(() => {
  ...
  const handleTrack = (e: React.FormEvent) => {
    ...
    if (trackingNumber.trim() === dummyTrackingData.trackingNumber) {
      navigate(`/track?trackingId=${trackingNumber.trim()}`);
            ...
            <div className="bg-white p-6 sm:p-10 rounded-3xl shadow-xl max-w-5xl w-full my-10">
              ...
              <div className="mt-10">
                ...
                <div className="h-64 sm:h-80 md:h-96 w-full rounded-2xl overflow-hidden shadow-md">
                  <MapContainer center={[8.0, 6.0]} zoom={6} style={{ height: "100%", width: "100%" }}>
                    <TileLayer
